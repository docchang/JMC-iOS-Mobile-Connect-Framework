...
//            [sprites addObject:img];
            ...
            NSString *sprintName = [@"icon_record_" stringByAppendingFormat:@"%d.tiff", i];
            [sprites addObject:CREATE_IMAGE(sprintName)];
        }
...
//    attachment.thumbnail = [UIImage imageNamed:@"audio_attachment"];
    attachment.thumbnail = CREATE_IMAGE(@"audio_attachment.tiff");
...
//    self.buttonOverlayView.image = [UIImage imageNamed:@"background_overlay.png"];
    self.buttonOverlayView.image = CREATE_IMAGE(@"background_overlay.tiff");
}
...
- (void)addScreenshotButton {
    if ([[JMC sharedInstance] isPhotosEnabled]) {
        self.screenshotButton = [self buttonFor:@"icon_capture.tiff" action:@selector(addScreenshot)];
...
- (void)addVoiceButton {
    if ([[JMC sharedInstance] isVoiceEnabled]) {
        self.voiceButton = [self buttonFor:@"icon_record.tiff" action:@selector(addVoice)];
...
//    [button setImage:[UIImage imageNamed:iconNamed] forState:UIControlStateNormal];
    [button setImage:CREATE_IMAGE(iconNamed) forState:UIControlStateNormal];
